...
class FeatureExtractor(object):
    ...
    def get_input_representation(self, words, pos, state):
        ...
        for i in word:
            if i is None: # <CD> 0 <NNP> 1 <UNK> 2 <ROOT> 3 <NULL> 4
                input_re.append(4)
            elif i == 0:
                input_re.append(3)
            elif pos[i] == 'NNP':
                ...
                word2ind = self.word_vocab.get(words[i].lower, None) # dict key(word) -> value(index) 
                if word2ind is None:
                    input_re.append(2) # if word not in word_vocabs corpus, add None
                else:
                    input_re.append(word2ind) # if word detected in word_vocabs corpus, add word's index
